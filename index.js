...
io.on("connection", (socket) => {
    ...
    socket.on("change_channel", async (packet, callback) => {
        ...
        if (data != null && data.rowCount == 1) {
            ...
            if (packet.channel_from != null) {
                socket.leave(packet.channel_from);
                io.to(packet.channel_from).emit(
                    "change_channel",
                    await controller.request_users({
                        channel_id: packet.channel_from,
                    })
                );
            }
            socket.join(packet.channel_to);
            io.to(packet.channel_to).emit(
                "change_channel",
                await controller.request_users({
    ...
    socket.on("create_room", async (packet, callback) => {
        ...
        if (data.rowCount == 1) {
            const room_id = data.rows[0].id;
            socket.join(room_id);
            callback({ result: true, room_id: room_id });
            io.to(packet.channel_id).emit(
                "create_room",
                await controller.request_rooms({
    ...
    socket.on("enter_room", async (packet, callback) => {
        ...
        if (data != null && data.rowCount == 1) {
            callback(true);
            io.to(packet.room_id).emit(
                "enter_room",
                controller.users_in_room({
    ...
    socket.on("ready", async (packet, callback) => {
        ...
        if (
            controller.users_in_room({ room_id: packet.room_id }).rowCount ==
